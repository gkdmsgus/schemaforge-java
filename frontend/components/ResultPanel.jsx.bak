...
function updateSavedResultChat(prompt, messages, graph) {
...
export function saveResultToLocal(result, prompt) {
  const saved = getSavedResults()
...
export default function ResultPanel({
  ...
  const [pcbFilename, setPcbFilename] = useState(null)
  const [gerberStatus, setGerberStatus] = useState(null)
  const [gerberInfo, setGerberInfo] = useState(null)
  const [activeTab, setActiveTab] = useState('Netlist')
  const [iterateText, setIterateText] = useState('')
  const [drawerOpen, setDrawerOpen] = useState(false)
  const [sidebarOpen, setSidebarOpen] = useState(true)
  const [viewMode, setViewMode] = useState('schematic')
  const [chatMessages, setChatMessages] = useState(() => initialChatSession?.messages || [])
  const [chatInput, setChatInput] = useState('')
  const [chatLoading, setChatLoading] = useState(false)
  const [localGraph, setLocalGraph] = useState(() => initialChatSession?.graph || null)
  const [graphHistory, setGraphHistory] = useState([])
  const [graphDiff, setGraphDiff] = useState(null) // { added, removed, modified } — sets of refs

  const effectiveGraph = localGraph || result?.graph

  function computeDiff(before, after) {
    const beforeRefs = new Set((before?.components || []).map(c => c.ref))
    const afterRefs  = new Set((after?.components  || []).map(c => c.ref))
    const added    = new Set([...afterRefs].filter(r => !beforeRefs.has(r)))
    const removed  = new Set([...beforeRefs].filter(r => !afterRefs.has(r)))
    const modified = new Set((after?.components || []).filter(c => {
      const old = (before?.components || []).find(o => o.ref === c.ref)
      return old && (old.value !== c.value || old.name !== c.name)
    }).map(c => c.ref))
    return { added, removed, modified }
  }

  function pushGraphHistory(g) {
  ...
  async function generateGerber() {
    ...
    try {
      ...
      if (data.error) throw new Error(data.error)
      setGerberInfo({ dir: data.gerberDir, files: data.files || [] })
  ...
  async function sendChat() {
    ...
    setChatMessages(prev => [...prev, userMsg, { role: 'assistant', content: '', _streaming: true, actions: [] }])
    setChatLoading(true)
        ...
        for (const part of parts) {
          ...
          if (evt === 'text') {
            ...
            setChatMessages(prev => {
              const next = [...prev]
              next[next.length - 1] = { role: 'assistant', content: streamedText, _streaming: true, actions: [] }
              return next
            })
            setTimeout(() => chatEndRef.current?.scrollIntoView({ behavior: 'smooth' }), 10)
          } else if (evt === 'done') {
            const { reply, actions } = JSON.parse(raw)
            const snapshot = effectiveGraph
            let newGraph = snapshot
            if (actions?.length) {
              pushGraphHistory(snapshot)
              newGraph = applyActions(snapshot, actions)
            ...
            setChatMessages(prev => [...prev.slice(0, -1), { role: 'assistant', content: `오류: ${message}`, actions: [] }])
          }
        }
      }
    } catch (e) {
      setChatMessages(prev => [...prev.slice(0, -1), { role: 'assistant', content: `오류: ${e.message}`, actions: [] }])
    }
  ...
  async function exportPDF() {
    ...
    const canvas = await html2canvas(el, { backgroundColor: '#07080a', scale: 2 })
  ...
  return (
    <div ref={resultRef} style={{
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden', minHeight: 0 }}>
        ...
        {/* LEFT — Chat AI editor */}
        <div style={{ width: 260, flexShrink: 0, borderRight: '1px solid var(--sf-line-soft)', background: 'var(--sf-bg-1)', display: 'flex', flexDirection: 'column' }}>
          {/* Header */}
          <div style={{ padding: '10px 14px', borderBottom: '1px solid var(--sf-line-soft)', display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <>
                ...
                {chatMessages.map((m, i) => (
                  <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: m.role === 'user' ? 'flex-end' : 'flex-start', gap: 3 }}>
                    <div style={{
                      ...
                      background: m.role === 'user' ? '#1c2a1a' : 'var(--sf-bg-1)',
                      border: `1px solid ${m.role === 'user' ? '#2a4020' : 'var(--sf-line)'}`,
                      fontFamily: 'var(--sf-font-sans)', fontSize: 11, color: m.role === 'user' ? '#7ce3a0' : 'var(--sf-fg-muted)', lineHeight: 1.5,
                    }}>
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3, maxWidth: '90%' }}>
                        {m.actions.filter(a => a?.type).map((a, j) => (
                          <span key={j} style={{
                            ...
                            background: a.type.startsWith('add') ? '#0f2010' : a.type.startsWith('remove') ? '#1a0f10' : 'var(--sf-bg-1)',
                            color: a.type.startsWith('add') ? '#34d3a0' : a.type.startsWith('remove') ? 'var(--sf-danger)' : 'var(--sf-violet)',
                            border: `1px solid ${a.type.startsWith('add') ? '#1a3020' : a.type.startsWith('remove') ? '#2a1515' : 'var(--sf-line)'}`,
                          }}>
              ...
              {/* Input */}
              <div style={{ padding: '8px 10px', borderTop: '1px solid var(--sf-line-soft)', display: 'flex', gap: 6, alignItems: 'flex-end' }}>
                <textarea
                  value={chatInput}
        ...
        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          {/* view tabs */}
          <div style={{ display: 'flex', alignItems: 'stretch', background: 'var(--sf-bg-1)', borderBottom: '1px solid var(--sf-line-soft)', flexShrink: 0 }}>
            {[
              ...
              return (
                <button key={key} onClick={() => setViewMode(key)} style={{
                  ...
                  borderRight: '1px solid var(--sf-line-soft)',
        ...
        {/* RIGHT — SPEC + BOM */}
        {sidebarOpen && (
          <div style={{ width: 240, flexShrink: 0, borderLeft: '1px solid var(--sf-line-soft)', background: 'var(--sf-bg-1)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            <div style={{ padding: '10px 14px', borderBottom: '1px solid var(--sf-line-soft)', display: 'flex', alignItems: 'center', gap: 6 }}>
              <span style={{ fontFamily: 'var(--sf-font-mono)', fontSize: 10, color: 'var(--sf-fg-faint)', letterSpacing: '0.12em', fontWeight: 700 }}>SPEC</span>
            </div>
            <div style={{ padding: '10px 14px', borderBottom: '1px solid var(--sf-line-soft)' }}>
              {[
            ...
            <div style={{ padding: '10px 14px 4px', borderBottom: '1px solid var(--sf-line-soft)' }}>
      ...
      <div style={{
        ...
        {/* drawer content */}
        {drawerOpen && (
          <div style={{ height: DRAWER_H, overflowY: 'auto', borderTop: '1px solid var(--sf-line-soft)' }}>
            {activeTab === 'Netlist' && (
            ...
            {activeTab === 'Sources' && result.sources?.length > 0 && (
...
function applyActions(graph, actions) {
  ...
  for (const a of actions) {
    if (a.type === 'add_component') {
      ...
      nets  = nets.map(n => ({ ...n, nodes: n.nodes.filter(nd => nd.ref !== a.ref) })).filter(n => n.nodes.length > 0)
...
function PanelLabel({ color, label, sub, extra }) {
  return (
    <div style={{
      ...
      padding: '0 14px',
      background: 'var(--sf-bg-1)',
      borderBottom: '1px solid var(--sf-line-soft)',
    }}>
...
function TimelineStrip({ versions, currentVersionId, onSelectVersion, onApplyVersion }) {
